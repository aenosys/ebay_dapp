...
const ipfs = ipfsAPI({ host: "localhost", port: "8000", protocol: "http" });
...
function renderProducts(div, filters) {
  $.ajax({
    ...
    if (data.length == 0) {
      ...
      if (div == "product-list") {
        $("#total-products").html(data.length);
      }
      $("#" + div).html("");
    }
    while (data.length > 0) {
...
function buildProduct(product) {
  ...
  node.append("<div><a href='/product.html?product-id=" + product["blockchainId"] + "'>" + product["name"] + "</a></div>");
...
function saveProductToBlockchain(params, imageId, descId) {
  ...
  EcommerceStore.deployed().then(function(i) {
    i.addProductToStore(
      ...
      .then(function(f) {
        console.log(f);
        $("#msg").fadeIn("Your product was successfully added to your store!");
      });
  });
}
  ...
  EcommerceStore.deployed().then(function(i) {
    i.getProduct.call(productId).then(function(p) {
      ...
      $("#product-image").append(
        "<img src='https://ipfs.io/ipfs/" + p[3] + "' width='250px' />"
      );
...
window.App = {
  start: function() {
    ...
    $("#bidding").submit(function(event) {
      ...
      let sealedBid =
        "0x" +
        ethUtil.sha3(web3.toWei(amount, "ether") + secretText).toString("hex");
      ...
      EcommerceStore.deployed().then(function(i) {
        i
          .revealBid(
            ...
            secretText,
            { from: web3.eth.accounts[1], gas: 440000 }
          )
          .then(function(f) {
            $("#msg").show();
      ...
      EcommerceStore.deployed().then(function(i) {
        i
          ...
          .catch(function(e) {
            ...
            $("#msg").show();
            $("#msg").html(
              "The auction can not be finalized by the buyer or seller, only a third party arbiter can finalize it"
            );
          });
      });
      event.preventDefault();
